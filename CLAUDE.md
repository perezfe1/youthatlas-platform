...
- **`force-dynamic` only on auth-gated pages.** Pages that read auth/searchParams MUST use `force-dynamic` (dashboard, login, `/opportunities/search`). Public pages use ISR or are fully static. Current ISR windows (don't shorten without checking Vercel usage first): homepage + `/dreamers-and-makers` = 6h, `/opportunities` listing = 4h, opportunity detail + SEO type/region pages = 7d.
- **Public reads MUST use `getPublicSupabaseClient()` (`src/lib/supabase/public.ts`), never `createServerSupabaseClient()`.** The server client's first line is `await cookies()` — a Next dynamic API that silently opts ISR pages into per-request rendering, and the `DynamicServerError` it throws gets swallowed by the services' `Result<T>` catch blocks, so nothing visibly breaks: pages just quietly build as `ƒ (Dynamic)` and every request burns Fluid CPU. This exact bug made ALL "ISR" pages render dynamically from May 25 to July 3, 2026 (63% of Hobby's monthly CPU cap). Only auth-touching code (search page badges, dashboard, profile, API routes with sessions) uses the cookie client. Verify after any change: `curl -sI https://youthatlas.com/opportunities` twice → second hit must show `x-vercel-cache: HIT`.

...
// This page is intentionally force-dynamic: it reads searchParams for
// filters/search and checks auth for "Matches you" profile badges.
// Only real users with active filters/search land here — bots hit
// /opportunities (ISR-cached) instead.
...
export default async function OpportunitiesSearchPage({ searchParams }: PageProps) {
  ...
  // Profile-based "Matches you" scoring for logged-in users.
    ...
    if (profileResult.data) {
      ...
      if (hasPreferences) {
        for (const opp of opportunities) {
          ...
          if (types_of_interest.length > 0 && types_of_interest.includes(opp.type)) score++;
          if (regions_of_interest.length > 0 && opp.regions.some((r) => regions_of_interest.includes(r))) score++;

          if (citizenships.length > 0) {
            const eligible = opp.eligible_nationalities ?? [];
            if (eligible.length > 0) {
              if (eligible.some((n) => citizenships.includes(n))) score++;
